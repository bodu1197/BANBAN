  ...
  return (
    <div className="flex items-center gap-2">
      <input
        ...
        aria-label="재생성할 토픽 ID"
        className="w-36 rounded-lg border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white placeholder:text-zinc-500 focus-visible:border-emerald-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      />
      <button
        type="button"
        disabled={running || !topicInput}
        onClick={() => onRun(Number(topicInput))}
        className="rounded-lg border border-white/10 bg-white/10 px-3 py-2.5 text-sm font-medium text-white hover:bg-white/20 focus-visible:bg-white/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50"
      >
        특정 토픽 재생성
      </button>
    </div>
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/5 p-5">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center">
        ...
        >
          {running ? <Loader2 className="h-4 w-4 motion-safe:animate-spin" /> : <PlayCircle className="h-4 w-4" />}
          다음 글 지금 생성
        </button>
  ...
  return (
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      ...
      <button
        type="button"
        onClick={onRefresh}
        className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white hover:bg-white/10 focus-visible:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
        <RefreshCw className={`h-4 w-4 ${loading ? "motion-safe:animate-spin" : ""}`} /> 새로고침
      </button>
    </div>
  );
}
  ...
  if (loading && !status) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="h-8 w-8 rounded-full border-2 border-indigo-500 border-t-transparent motion-safe:animate-spin" />
      </div>
    );
  }
