...
export function EventStepPhotos({
  ...
  const handleFileSelect = useCallback(
    (index: number, file: File | null) => {
      const next = [...slots];
      // eslint-disable-next-line security/detect-object-injection -- index 는 slots.map 의 숫자 인덱스
      ...
      if (file) {
        ...
        // eslint-disable-next-line security/detect-object-injection -- index 는 slots.map 의 숫자 인덱스
        next[index] = { ...current, file: null, preview: "" };
      }
      onSlotsChange(next);
    },
    [slots, onSlotsChange],
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4">
        {slots.map((slot, i) => (
          <PhotoSlotCard
            ...
            slot={slot}
            registerRef={(el) => {
              // eslint-disable-next-line security/detect-object-injection -- i 는 slots.map 의 숫자 인덱스
            ...
            onChange={(file) => handleFileSelect(i, file)}
      ...
      {/* Navigation */}
      <PhotoNav allFilled={allFilled} onBack={onBack} onNext={onNext} />
    </div>
  );
}
