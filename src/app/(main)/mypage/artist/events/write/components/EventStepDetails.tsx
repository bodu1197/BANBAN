...
type OnChange = (updates: Partial<EventFormValues>) => void;

function EventPeriodFields({
  values,
  onChange,
}: Readonly<{ values: EventFormValues; onChange: OnChange }>): React.ReactElement {
  return (
    <div>
...
function AdvantageInputs({
  ...
  return (
    <div>
      ...
      {[0, 1, 2].map((i) => {
        // eslint-disable-next-line security/detect-object-injection -- i는 [0,1,2] 리터럴 배열의 숫자 인덱스
        const advantage = values.procedureAdvantages[i];
        return (
          <input
            key={i}
            id={`evt-advantage-${i}`}
...
function OptionalFields({
  ...
  return (
    <div className="space-y-4 rounded-lg border border-dashed border-input p-4">
      ...
      <AdvantageInputs values={values} onChange={onChange} />
...
export function EventStepDetails({
  ...
  return (
    <div className="space-y-5">
      <EventPeriodFields values={values} onChange={onChange} />
      <SummaryField values={values} onChange={onChange} />
      <TargetAudienceField values={values} onChange={onChange} toggleTarget={toggleTarget} />
      <ShopInfoFields values={values} onChange={onChange} artistTitle={artistTitle} />
      ...
      {showOptional && <OptionalFields values={values} onChange={onChange} />}

      <StepNavigation canProceed={canProceed} onNext={onNext} onBack={onBack} />
    </div>
  );
}
