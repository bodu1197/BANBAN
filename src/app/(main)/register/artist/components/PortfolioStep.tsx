...
function ProgressHint({ reachedGate, reachedMin, minRequired }: Readonly<{
  ...
  if (reachedGate) {
    return <p className="mt-2 text-xs font-medium text-emerald-600">검수 신청 기준({REQUIRED_PORTFOLIOS}개)을 충족했어요! ‘완료’를 누르면 자동으로 검수 신청됩니다.</p>;
  }
  if (reachedMin) {
    return <p className="mt-2 text-xs text-muted-foreground">‘완료’를 눌러 진행할 수 있어요. 더 채우면 바로 검수 신청까지 됩니다.</p>;
  }
    ...
    <div className="space-y-6 p-4">
      ...
      <div className="rounded-xl border border-brand-primary/30 bg-brand-primary/5 p-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-xs text-muted-foreground tabular-nums">검수 신청 기준 {count}/{REQUIRED_PORTFOLIOS}</span>
        </div>
        ...
        <p className="mt-2 flex items-start gap-1.5 text-xs text-brand-primary">
          ...
          <span>작품을 많이 올릴수록 상담 문의가 비례해서 늘어나요. 최소 {minRequired}개부터 시작해 {REQUIRED_PORTFOLIOS}개까지 채우면 검수 신청이 가능합니다.</span>
        </p>
