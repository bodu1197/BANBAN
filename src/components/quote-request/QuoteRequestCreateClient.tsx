// @client-reason: form with state management for creating quote requests
"use client";
...
function useQuoteForm() {
  ...
  function handleSubmit(e: React.FormEvent): void {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        router.push(`/quote-request/${result.id}`);
      } else {
...
export function QuoteRequestCreateClient({ labels}: Readonly<Props>): React.ReactElement {
...
function QuoteFormFields({ f, labels }: Readonly<{
  f: ReturnType<typeof useQuoteForm>;
  labels: Record<string, string>;
}>): React.ReactElement {
  return (
    <>
...
function QuoteForm({ f, labels }: Readonly<{
  f: ReturnType<typeof useQuoteForm>;
  labels: Record<string, string>;
}>): React.ReactElement {
  return (
    <form onSubmit={f.handleSubmit} className="flex flex-col gap-5">
      <QuoteFormFields f={f} labels={labels} />
      <div>
