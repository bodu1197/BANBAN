...
export async function generateBlogMetadata(): Promise<Metadata> {
  const title = "반영구 블로그 — 작품 해석과 트렌드";
  const description = "전문가의 시선으로 바라본 반영구 작품 해석, 스타일 분석, 트렌드 가이드. 눈썹, 아이라인, 입술 등 다양한 반영구 정보를 확인하세요.";
    ...
    <div className="mx-auto w-full max-w-[767px]">
      <header className="border-b border-border px-4 py-4">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {`전문가의 시선으로 해석한 ${count.toLocaleString()}개의 반영구 작품`}
        </p>
      </header>
      <BlogSearchClient
