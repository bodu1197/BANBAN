...
const PROVIDER_LABEL_BY_TYPE: Readonly<Record<SocialProvider, string>> = {
  NONE: "이메일",
  GOOGLE: "구글",
  KAKAO: "카카오",
  APPLE: "애플",
...
const PROVIDER_LABEL_BY_SLUG: Readonly<Record<ProviderSlug, string>> = {
  email: "이메일",
  google: "구글",
  kakao: "카카오",
  apple: "애플",
...
export function getProviderLabel(typeSocial: string | null | undefined): string {
  return PROVIDER_LABEL_BY_TYPE[normalizeTypeSocial(typeSocial)];
}
...
export function getLabelFromSlug(slug: string | null | undefined): string {
  switch (slug) {
    case "google": return PROVIDER_LABEL_BY_SLUG.google;
    case "kakao": return PROVIDER_LABEL_BY_SLUG.kakao;
    case "apple": return PROVIDER_LABEL_BY_SLUG.apple;
    default: return PROVIDER_LABEL_BY_SLUG.email;
  }
}
